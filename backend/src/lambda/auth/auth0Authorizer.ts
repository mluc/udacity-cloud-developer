...
const jwksUrl = 'https://dev-mluc.auth0.com/.well-known/jwks.json';
...
async function verifyToken(authHeader: string): Promise<JwtPayload> {
    ...
    const signingKeys = jwks
        ...
        .map(key => {
            ...
            return {cert: certToPEM(key.x5c[0]) };
        });
    return verify(token, signingKeys[0].cert, { algorithms: ['RS256'] }) as JwtPayload
